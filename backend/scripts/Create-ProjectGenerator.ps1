...
# Validation
if (-not (Test-ValidProjectName -Name $RepoName)) {
    Write-Log "Error: Invalid project name '$RepoName'. Use only alphanumeric characters, hyphens, and underscores (max 64 chars)." -Level "ERROR"
    exit 1
}

if ($Destination -and -not (Test-ValidPath -Path $Destination)) {
    Write-Log "Error: Invalid destination path '$Destination'." -Level "ERROR"
    exit 1
}
